...
uint8_t ModbusMaster::ModbusMasterTransaction(uint8_t u8MBFunction)
{
  ...
  while (u8BytesLeft && !u8MBStatus)
  {
    ...
    if (u8ModbusADUSize == (5 + 3)) //////////////////////////// mexido
    {
      // verify response is for correct Modbus slave
      if (u8ModbusADU[3] != _u8MBSlave) //////////////////////////// mexido
      {
      ...
      if ((u8ModbusADU[4] & 0x7F) != u8MBFunction) //////////////////////////// mexido
      {
      ...
      if (bitRead(u8ModbusADU[4], 7)) //////////////////////////// mexido
      {
        u8MBStatus = u8ModbusADU[5]; //////////////////////////// mexido
        break;
      }
      
      // evaluate returned Modbus function code
      switch(u8ModbusADU[4]) //////////////////////////// mexido
      {
        case ku8MBReadCoils:
        case ku8MBReadDiscreteInputs:
        case ku8MBReadInputRegisters:
        case ku8MBReadHoldingRegisters:
        case ku8MBReadWriteMultipleRegisters:
          u8BytesLeft = u8ModbusADU[5]; //////////////////////////// mexido
          break;
  ...
  if (!u8MBStatus && u8ModbusADUSize >= (5 +3)) //////////////////////////// mexido
  {
    // calculate CRC
    u16CRC = 0xFFFF;
    for (i = 3; i < (u8ModbusADUSize - 2); i++) //////////////////////////// mexido
    {
      u16CRC = crc16_update(u16CRC, u8ModbusADU[i]);
    }
  ...
  {
    // evaluate returned Modbus function code
    switch(u8ModbusADU[3]) //////////////////////////// mexido
    {
      case ku8MBReadCoils:
      case ku8MBReadDiscreteInputs:
        // load bytes into word; response bytes are ordered L, H, L, H, ...
        for (i = 0; i < (u8ModbusADU[5] >> 1); i++) //////////////////////////// mexido
        {
          if (i < ku8MaxBufferSize)
          {
            _u16ResponseBuffer[i] = word(u8ModbusADU[2 * i + 4 + 3], u8ModbusADU[2 * i + 3 + 3]); //////////////////////////// mexido
          }
        ...
        if (u8ModbusADU[2] % 2)
        {
          if (i < ku8MaxBufferSize)
          {
            _u16ResponseBuffer[i] = word(0, u8ModbusADU[2 * i + 3 + 3]);   //////////////////////////// mexido
          }
      ...
      case ku8MBReadWriteMultipleRegisters:
        ...
        for (i = 0; i < (u8ModbusADU[5] >> 1); i++) //////////////////////////// mexido
        {
          if (i < ku8MaxBufferSize)
          {
            _u16ResponseBuffer[i] = word(u8ModbusADU[2 * i + 3 +3], u8ModbusADU[2 * i + 4 +3]); //////////////////////////// mexido
          }
